...
def sigmoid(inX):
    ...
    if inX >= 0:
        ...
        return numpy.exp(inX) / (1 + numpy.exp(inX))
...
# # weights = stocGradAscent0(numpy.array(dataArr), labelMat)  # 随机梯度上升
# weights = stocGradAscent1(numpy.array(dataArr), labelMat)  # 改进的随机梯度上升
# plotBestFit(weights)
